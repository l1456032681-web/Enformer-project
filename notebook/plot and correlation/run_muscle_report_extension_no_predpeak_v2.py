...
from overlay_plotting_no_predpeak_v2 import plot_four_gene_overview, plot_gene_overlay
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="One or more comma-separated genes to visualize.",
    )
...
def main() -> None:
    ...
    if len(four_gene_panels) >= 1:
        ordered_panels = [next(panel for panel in four_gene_panels if panel["gene"] == g) for g in figure_genes if any(panel["gene"] == g for panel in four_gene_panels)]
        fig = plot_four_gene_overview(ordered_panels)
        overview_name = "four_gene_overview.png" if len(ordered_panels) == 4 else "multi_gene_overview.png"
        fig.savefig(report_dir / overview_name, dpi=220, bbox_inches="tight")
        plt.close(fig)
